...
export const createCss = function (compObj: any) {
    ...
    const cssRules = css.map(item => {
        ...
        // 如果没有 CSS 属性，返回空字符串
        if (cssProperties.length === 0 && !value.customCss) return '';
        
        let cssPropertiesStr = '';
        
        // 处理自定义 CSS - 改为字符串处理
        if (value.customCss && typeof value.customCss === 'string') {
            const lines = value.customCss.split('\n');
            const customCssWithoutFirstLine = lines.join('\n');
            // 如果有常规CSS属性，将它们合并到自定义CSS中
            if (cssProperties.length > 0) {
                cssPropertiesStr = `.comp_${key}{
            ${cssProperties.join(';')};
            ${customCssWithoutFirstLine}
        }`;
            } else {
                // 只有自定义CSS
                cssPropertiesStr = `.comp_${key}{${customCssWithoutFirstLine}
        }`;
            }
        } else {
            // 只有常规CSS属性
...
/**
 * 验证和格式化自定义CSS字符串
 * @param cssString 输入的CSS字符串
 * @returns 格式化后的CSS字符串
 */
function validateAndFormatCustomCss(cssString: string): string {
