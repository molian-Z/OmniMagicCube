...
const initializeAttribute = (prop: PropConfig): ComponentAttribute => {
    ...
    if (prop.default !== undefined) {
        return {
            type: prop.type || getPropertyType(prop.default),
            value: prop.default
        };
    }
...
export const ensureAnimationsStructure = (comp: any): void => {
    if (!comp.animations) {
        ...
        // 确保stateChange结构存在
        if (!comp.animations.stateChange) {
            comp.animations.stateChange = {
                active: [],
                inactive: [],
                selected: [],
                unselected: [],
            ...
            // 确保常见状态存在
            const commonStates = ['active', 'inactive', 'selected', 'unselected', 'expanded', 'collapsed', 'loading', 'loaded', 'visible', 'hidden', 'enabled', 'disabled', 'focused', 'blurred'];
            commonStates.forEach(state => {
                if (!comp.animations.stateChange[state]) {
                    comp.animations.stateChange[state] = [];
                }
            });
        }
        
        // 确保interaction结构存在
        if (!comp.animations.interaction) {
            comp.animations.interaction = {
                hover: [],
                click: [],
                focus: [],
                blur: [],
            ...
            // 确保常见交互事件存在
            const commonEvents = ['hover', 'click', 'focus', 'blur', 'mouseenter', 'mouseleave', 'mousedown', 'mouseup', 'keydown', 'keyup', 'scroll', 'resize'];
            commonEvents.forEach(event => {
                if (!comp.animations.interaction[event]) {
                    comp.animations.interaction[event] = [];
                }
            });
        }
    }
...
export const ensureBasicStructure = (comp: any): void => {
    ...
    // 确保css结构完整性
    if (comp.css && typeof comp.css === 'object') {
        const defaultCss = initCss();
        // 合并默认CSS配置，确保所有必要属性都存在
        comp.css = { ...defaultCss, ...comp.css };
        
        // 确保units对象存在且包含默认配置
        if (!comp.css.units || typeof comp.css.units !== 'object') {
            comp.css.units = defaultCss.units;
        } else {
            comp.css.units = { ...defaultCss.units, ...comp.css.units };
        }
    }
}
