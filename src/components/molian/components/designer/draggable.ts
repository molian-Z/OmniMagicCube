...
export const useDraggable = (comps?: any, compData?: any, message?: any) => {
    ...
    const onDrop = function (evt: any, index: any, slotVal: any) {
        ...
        if (slotVal && slotVal.allowComps && allowCompsBtn(slotVal, name) || slotVal && !slotVal.allowComps || !slotVal) {
            ...
            if (!!isCreate) {
                const comp = slotVal.appendComps && slotVal.appendComps.find((item:any) => {
    ...
    const onDropSlot = function (evt: any, slotVal: any) {
        const name = evt.dataTransfer.getData('compName')
    ...
    const allowCompsBtn = (slotVal:any, name:string) => {
        ...
        return slotVal.allowComps.length === 0 || compIndex > -1
    }
