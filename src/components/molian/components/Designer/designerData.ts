...
/**
 * 查找距离最近的component-wrapper元素的下一层有宽高的元素
 * @param target 起始元素
 * @returns 找到的有宽高的元素或null
 */
const findNearestSizedElement = (target: HTMLElement | null): HTMLElement | null => {
    if (!target) return null;
    
    // 向上查找最近的component-wrapper元素
    let componentWrapper = target.closest('.component-wrapper');
    if (!componentWrapper) return null;
    
    // 在component-wrapper的子元素中查找有宽高的元素
    const findSizedChild = (element: Element): HTMLElement | null => {
        // 检查当前元素是否有宽高
        ...
        for (const child of Array.from(element.children)) {
            const result = findSizedChild(child);
    ...
    return findSizedChild(componentWrapper);
};
...
export const setSelectedComp = (comp: any, evt: any) => {
    ...
    selectedDom.value = evt ? findNearestSizedElement(evt.target as HTMLElement) : null;
...
/**
 * 高性能的CSS属性更新函数
 * 同时更新selectedComp和modelValue中的对应组件，确保数据一致性
 * @param prop CSS属性名
 * @param val CSS属性值
 */
export const updateSelectedCompCss = (prop: string, val: any) => {
    if (!selectedComp.value) return;
    
    // 确保CSS对象存在
    if (!selectedComp.value.css) {
        selectedComp.value.css = reactive({});
    }
    if (!selectedComp.value.css.units) {
        selectedComp.value.css.units = reactive({});
    }
    
    // 更新选中组件的CSS
    selectedComp.value.css[prop] = val;
    
    // 同步更新modelValue中的对应组件
    if (modelValue.value && selectedComp.value.id) {
        syncComponentInTree(modelValue.value, selectedComp.value.id);
    }
    
    // 强制触发响应式更新 - 通过创建新的数组引用来触发深层监听
